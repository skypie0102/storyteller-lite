...
- Canonical Rust + Slint implementation branch: `main`.
- `recovery/rust-slint` is retained as reconstruction history; `main` is current implementation truth.
...
- Manual acceptance evidence now also exists for the three published interoperability EPUB artifacts: all three were tested and reported to work normally. The reader/version was not recorded, so keep this as manual acceptance evidence rather than a reproducible reader-specific gate.
...
- All temporary validation PRs through this checkpoint are closed without merge. Validated product/docs changes are integrated into `main`; temporary validation workflows, patch helpers, and validation-only merge commits are not treated as implementation truth.
- A public portable-release path now exists for explicit `release/vMAJOR.MINOR.PATCH` branches. The release workflow requires Cargo/package version agreement, reruns the full Windows release gate, verifies provenance and hashes, exercises packaged relaunch recovery, and creates a GitHub Release only after all checks pass. The executable remains unsigned; no installer or auto-update channel exists yet.
- Public `v0.1.0` is released from commit `f208abbd9ee27eb19cb3ed2f802a8ecda17c38c8`. Windows release run `35302413893` passed release identity, rustfmt, strict workspace Clippy, all workspace tests, the locked release build, package provenance/hash checks, packaged relaunch recovery, archive verification, artifact upload, and GitHub Release publication. Tag `v0.1.0` points to that exact commit. The release assets are `StoryTeller-Lite-v0.1.0-windows-x64.zip` plus its `.sha256`; the executable is unsigned and there is no installer or auto-update channel.
